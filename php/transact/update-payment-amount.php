...
// Fetch tenant name and unit color based on tenant_id
...
if ($result->num_rows > 0) {
    // Fetch tenant name and unit color
    $tenant = $result->fetch_assoc();
    $tenant_name = $tenant['name'];
    $unit_color = $tenant['unit_color'];

    // Update tenant payment details
    ...
    if ($stmtUpdate->execute()) {
        // Insert into payment history including tenant name and unit color
